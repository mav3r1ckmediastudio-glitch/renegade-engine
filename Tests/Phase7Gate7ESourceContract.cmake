...
require_text("${inspector}" "std::filesystem::file_size" "cheap video size preflight")
require_text("${inspector}" "CreatorVideoWorkflowService::MaximumCreatorVideoBytes" "creator video size ceiling before decode")
require_text("${inspector}" "wi::video::CreateVideo(filename, &probe)" "pre-commit Wicked H264 decode preflight")
string(FIND "${inspector}" "std::filesystem::file_size" size_preflight_pos)
string(FIND "${inspector}" "wi::video::CreateVideo(filename, &probe)" decode_preflight_pos)
if(size_preflight_pos EQUAL -1 OR decode_preflight_pos EQUAL -1 OR
   NOT size_preflight_pos LESS decode_preflight_pos)
    message(FATAL_ERROR "Phase 7E video size ceiling must run before full Wicked decode preflight")
endif()
