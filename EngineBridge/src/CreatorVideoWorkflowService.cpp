...
namespace
{
    ...
    CreatorVideoImportResult Failure(std::string error)
    {
        CreatorVideoImportResult result;
        ...
        std::string& error)
    {
        ...
        options.transactionId =
            "phase7-video-registry-bootstrap-" + GenerateStableId();
...
{
    CreatorVideoImportResult CreatorVideoWorkflowService::ImportVideo(
        const std::string& projectRoot,
        const StableId& projectId,
        const std::string& externalSourcePath) const
    {
        if (projectRoot.empty() || !IsValidStableId(projectId))
            return Failure("Video import requires an active Renegade project.");
        if (externalSourcePath.empty())
            return Failure("Video import requires a source file.");
        ...
        if (ec || external.empty() || !fs::is_regular_file(external, ec) || ec)
            return Failure("The selected video source is unavailable.");
        ...
        if (format == ResourceSourceFormat::Unknown ||
            ClassifyResourceSourceFormat(format) != ResourceClass::Video)
        {
            return Failure(
                "The selected file is not a video format supported by Renegade's pinned Wicked resource manager.");
        }

        const std::uintmax_t sourceBytes = fs::file_size(external, ec);
        if (ec)
            return Failure("Could not inspect the selected video size: " + ec.message());
        if (sourceBytes == 0)
            return Failure("The selected video source is empty.");
        if (sourceBytes > MaximumCreatorVideoBytes)
            return Failure("The selected video exceeds Renegade's 4 GiB creator-import ceiling.");
        ...
        const fs::path sourceDirectory = root / "SourceAssets" / "Video";
        const fs::path productDirectory = root / "Content" / "Video";
        fs::create_directories(sourceDirectory, ec);
        if (ec)
            return Failure("Could not create SourceAssets/Video: " + ec.message());
        fs::create_directories(productDirectory, ec);
        if (ec)
            return Failure("Could not create Content/Video: " + ec.message());

        std::string stem = external.stem().generic_u8string();
        if (stem.empty())
            stem = "ImportedVideo";
        const std::string extension = LowerAscii(external.extension().generic_u8string());
        if (extension.empty())
            return Failure("The selected video has no supported extension.");
        ...
        for (std::uint32_t index = 1; index <= MaximumNameAttempts; ++index)
        {
            ...
            retainedSource = sourceDirectory / fs::u8path(candidateStem + extension);
            ...
            if (ec)
                return Failure("Could not inspect the retained video destination.");
            const bool productExists = fs::exists(governedProduct, ec);
            if (ec)
                return Failure("Could not inspect the governed video destination.");
        ...
        if (!uniqueDestinationFound)
            return Failure("Could not allocate a unique video name after 1024 attempts.");
        ...
        if (ec)
        {
            ...
            {
                return Failure(
                    "The retained video destination became occupied during import; retry the import.");
            }
            return Failure("Could not retain the original video source: " + ec.message());
        }
        ...
        if (!result.asset.succeeded)
        {
            result.error = result.asset.error.empty()
                ? "The governed video transaction failed."
                : result.asset.error;
