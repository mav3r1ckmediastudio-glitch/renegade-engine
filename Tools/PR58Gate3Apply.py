...
create_hub = r'''    void StudioRenderPath::CreateProjectHub()
    {
...
replace_regex(
    r'    void StudioRenderPath::CreateProjectHub\(\)\n    \{.*?\n    \}\n\n    // A small, self-contained popup',
    create_hub + '\n\n    // A small, self-contained popup',
    'CreateProjectHub')
...
replace_exact(
    ...
    'hub label/button theme')

hub_layout = r'''        const float hubMargin = std::clamp(width * 0.025f, 24.0f, 40.0f);
...
replace_regex(
    r'        projectHubPanel_\.SetPos\(XMFLOAT2\(12\.0f, 12\.0f\)\);.*?'
    r'        hubMessageLabel_\.SetSize\(XMFLOAT2\(width - 90\.0f, 38\.0f\)\);',
    hub_layout,
    'Project Hub ResizeLayout block')
