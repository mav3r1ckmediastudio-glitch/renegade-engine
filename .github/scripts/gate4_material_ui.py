...
# ---------------------------------------------------------------------------
# StudioApplication.h: declarations/state only. Implementation stays isolated.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# StudioApplication.cpp: only bounded integration hooks.
# ---------------------------------------------------------------------------
source = "Studio/src/StudioApplication.cpp"
replace_once(
    source,
...
) if '            cameraSliderActive_ ||\n            lightPlacementActive_ ||\n' in read(source) else None

# Older shortcut guard does not explicitly mention camera sliders. Keep the
# bounded material guard adjacent to the existing light-slider guard instead.
if '            materialSliderActive_ ||\n' not in read(source):
    replace_once(
        source,
        '''            lightSliderActive_ ||\n            lightPlacementActive_ ||\n''',
        '''            lightSliderActive_ ||\n            materialSliderActive_ ||\n            lightPlacementActive_ ||\n''',
...
# ---------------------------------------------------------------------------
# Dedicated Gate 4 material Inspector implementation.
# ---------------------------------------------------------------------------
material_cpp = r'''#include "StudioApplication.h"
...
'''
write("Studio/src/Phase5Gate4MaterialInspector.cpp", material_cpp)

# Studio target owns the new dedicated implementation unit.
...
# Basic bounded self-check before the workflow removes itself.
for path in [
    header,
    source,
    "Studio/src/Phase5Gate4MaterialInspector.cpp",
    cmake,
]:
...
for token in [
    ...
    if token not in read("Studio/src/Phase5Gate4MaterialInspector.cpp"):
        raise RuntimeError(f"new material inspector missing token {token}")

print("Gate 4 material Inspector patch applied")
