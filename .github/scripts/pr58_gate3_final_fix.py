...
pattern = re.compile(r'    void RenegadeProjectHub::Render\(\n.*?\n    \}\n\}', re.S)
match = pattern.search(hub)
assert match, 'Gate 3 Render() anchor drifted'
hub = hub[:match.start()] + new_render + '\n}' + hub[match.end():]

old_open = '            case RenegadeProjectHub::Action::OpenProject: OpenProject(); break;'
new_open = '''            case RenegadeProjectHub::Action::OpenProject:\n                // Defer the native browser until the next thread-safe point.\n                // Calling it directly from the custom Hub input pass can race\n                // the GUI/input update and was observed by the owner as a\n                // dead OPEN PROJECT control.\n                hubMessageLabel_.font.params.color = HologramMuted;\n                hubMessageLabel_.SetText("PROJECT BROWSER // OPENING");\n                wi::eventhandler::Subscribe_Once(\n                    wi::eventhandler::EVENT_THREAD_SAFE_POINT,\n                    [this](uint64_t) { OpenProject(); });\n                break;'''
assert app.count(old_open) == 1, 'Open Project action anchor drifted'
app = app.replace(old_open, new_open)

old_dialog_callback = '''            [this](const std::string& descriptorPath)\n            {\n                wi::eventhandler::Subscribe_Once(\n                    wi::eventhandler::EVENT_THREAD_SAFE_POINT,\n                    [this, descriptorPath](uint64_t)\n                    {\n                        OpenProjectDescriptor(descriptorPath);\n                    });\n            });'''
new_dialog_callback = '''            [this](const std::string& descriptorPath)\n            {\n                if (descriptorPath.empty())\n                    return;\n                wi::eventhandler::Subscribe_Once(\n                    wi::eventhandler::EVENT_THREAD_SAFE_POINT,\n                    [this, descriptorPath](uint64_t)\n                    {\n                        hubMessageLabel_.font.params.color = HologramMuted;\n                        hubMessageLabel_.SetText(\n                            "PROJECT OPENING // " +\n                            wi::helper::GetFileNameFromPath(descriptorPath));\n                        OpenProjectDescriptor(descriptorPath);\n                    });\n            });'''
# The same callback shape can occur elsewhere; limit the replacement to the
# OpenProject() function body by slicing between stable function anchors.
start = app.index('    void StudioRenderPath::OpenProject()')
end = app.index('    void StudioRenderPath::RequestSceneReplacement(', start)
segment = app[start:end]
assert segment.count(old_dialog_callback) == 1, 'OpenProject FileDialog callback anchor drifted'
segment = segment.replace(old_dialog_callback, new_dialog_callback)
app = app[:start] + segment + app[end:]

HUB.write_text(hub, encoding='utf-8', newline='\n')
APP.write_text(app, encoding='utf-8', newline='\n')

# Remove the one-shot patch machinery before committing the real fix.
Path('.github/scripts/pr58_gate3_final_fix.py').unlink(missing_ok=True)
Path('.github/workflows/pr58-gate3-final-fix.yml').unlink(missing_ok=True)
