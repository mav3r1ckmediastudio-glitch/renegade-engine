## A6 native one-shot variants - 22 September 2026

On `feature/a6-animation-wiring-v4`, code checkpoint `ef8dcce1bec8af8529877484bc665738d2f9debd` selects authored native Attack/Reload/Hit/Death variants using independent, reproducible, character-ID-seeded sequences; no immediate repeat, no variant consumed by interrupted/repeated active one-shots. Release Runtime/AI tests/graphics proof and Studio build pass; focused CTest 4/4 and real owner Mutant import/reopen/native transition/melee proof pass. Exact V4 UI remains frozen at tag `importer-ui-v4-approved-20260922` on `5152874`; no TestGame or owner V6 visual validation yet. See `docs/CHARACTER_IMPORTER_V4_WIRING_HANDOFF.md` for verification and remaining functionality.
## V4 animation wiring checkpoint - 22 September 2026

`feature/a6-animation-wiring-v4` code checkpoint `85408871aecead786fca52692e151b3796db73c4`: native Start/End edits are bounded to their real source clips, invalid trim/speed errors appear in the visible V4 Validation card, and a real-Mutant governed graphics proof now verifies duplicate Walk segment save/reopen (range, speed and action metadata) plus native A6 transitions and autonomous combat. Release Studio and proof builds pass; focused CTest 4/4, exact V4 UI source guard and diff check pass. Separate `RenegadeStudio_ImporterWiring_v5.exe` has not been visually accepted or run in TestGame. Frozen V4 source tag `importer-ui-v4-approved-20260922` and `main` unchanged. See `docs/CHARACTER_IMPORTER_V4_WIRING_HANDOFF.md` for evidence, limits and next step.
